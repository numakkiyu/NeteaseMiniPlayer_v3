...
Astro 的 Islands 架构允许在静态 HTML 中直接输出 `<nmp-player>`。NMPv3 的 ESM 包可以在 SSR 构建阶段安全解析，但自动注册入口必须通过 Astro 客户端脚本执行。
...
Astro 会把普通 `<script>` 作为客户端模块打包，浏览器加载后注册 `<nmp-player>`。
...
- 确认在普通 `<script>` 中导入 `netease-mini-player-v3/auto`
- 不要依赖 frontmatter 导入向浏览器注入自动注册脚本

### 自定义元素未注册

- 检查 `import "netease-mini-player-v3/auto"` 是否位于客户端 `<script>` 中
- 确保包含该脚本的 Astro 组件出现在最终页面
...
对于需要 SSR 的页面，仍可在 `.astro` 组件中输出 `<nmp-player>`。服务端只生成 HTML，自动注册入口由客户端脚本执行。
